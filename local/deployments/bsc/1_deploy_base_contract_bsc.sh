...
function hardhat_deploy_base(){
  ...
  if [ "$1" = "ACCESS_MANAGER_ADDRESS" ]; then
    yarn hardhat deployAcessManager --network $BSC_NETWORK_NAME --wallet $SUPER_ADMIN
  fi
  ...
  if [ "$1" = "grantRole" ]; then
    yarn hardhat grantRole --role $MULTISEND_ROLE --to $MULTICALl_ADDRESS --network $BSC_NETWORK_NAME
  fi
  ...
  if [ "$1" = "hasRole" ]; then
    yarn hardhat hasRole --role $MULTISEND_ROLE --to $MULTICALl_ADDRESS --network $BSC_NETWORK_NAME
  fi
...
rm -rf ../../../../local/env_bsc_base.txt
mv env.txt ../../../../local/env_var/env_bsc_base.txt
